...
rock.perm(601:700)  = 13*milli*darcy();
...
%                 'Val', 0, 'Radius', .12*meter, 'name', 'I');
% disp('Well #1: '); display(W(1));
% 
% W = verticalWell(W, G, rock, nx, ny, 1:nz, 'Type', 'bhp', 'Comp_i', 1,...
%                 'Val', 100*barsa, 'Radius', .12*meter, 'name', 'P');
...
newplot;
plotFaces(G, 1:G.faces.num, convertTo(state.facePressure, barsa()));
...
% sol = simpleIncompTPFA(initResSol(G, 0.0), G, T, fluid, 'wells', W);
% 
