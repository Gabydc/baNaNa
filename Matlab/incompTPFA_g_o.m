function [state, report] = incompTPFA_g_o(state, G, T, fluid, varargin)
%Solve incompressible flow problem (fluxes/pressures) using TPFA method.
%
% SYNOPSIS:
%   [state, report] = incompTPFA(state, G, T, fluid)
%   [state, report] = incompTPFA(state, G, T, fluid, 'pn1', pv1, ...)
%
% DESCRIPTION:
...
%
%  report - Struct containing information about the linear solver, it contains
%               - flag     -- returns a convergence FLAG:
%                             0 method converged to the desired tolerance TOL 
%                             within MAXIT iterations
%                             1 method iterated MAXIT times but did not converge.
%                             2 preconditioner M was ill-conditioned.
%                             3 method stagnated (two consecutive iterates were the same).
%                             4 one of the scalar quantities calculated 
%                             during ls became too small or too large to 
%                             continue computing.flag, ...
%               - residual    returns the relative residual
%                             NORM(B-A*X)/NORM(B). If FLAG is 0, then 
%                             RELRES <= TOL.res,...
%               - iterations  returns the iteration number
...
%}
   ...
   %lf = functions(opt.LinSolve);
   %if lf.function == 'mldivide'
   %    [p] = opt.LinSolve(A, rhs);
   %else
       [p, report] = opt.LinSolve(A, rhs);
   %end
   tocif(opt.Verbose, t0);
